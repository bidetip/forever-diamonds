...
def nueva_venta(venta: NuevaVenta):
    stock = database.get_stock(venta.codigo_cip)
    if stock <= 0:
        raise HTTPException(status_code=400, detail=f"Sin stock disponible para {venta.codigo_cip}")
    ...
    return {**resultado, "precio_venta": precio_venta, "precio_final": precio_final, "mensaje": f"Venta {resultado['id_venta']} registrada correctamente"}
